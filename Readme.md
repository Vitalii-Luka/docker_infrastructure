...
2. `Docker infrastructure` (this repository) - run [Traefik](https://traefik.io/) reverse-proxy container with linked
   ...
   [Ubuntu post-installation scripts](https://github.com/Vitalii-Luka/ubuntu_post_install_script). Check this repository
...
We strongly recommend using the `SETUP`, `DOCKERIZE` and `ENVADD` aliases from the [Dockerizer for PHP](https://github.com/Vitalii-Luka/dockerizer_for_php)
